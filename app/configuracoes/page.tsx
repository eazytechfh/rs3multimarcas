...
export default function Configuracoes() {
  ...
  const handleMembersUpdate = () => {
    if (user) loadCompanyMembers(user.id_empresa)
  }
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-white via-[#F6F2FF] to-[#EEF4FF]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden lg:ml-0">
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-transparent">
          <div className="container mx-auto px-6 py-8">
            <div className="mb-8">
              <h1 className="text-3xl font-bold mb-2 text-[#7C3AED]">Configurações</h1>
              <p className="text-[#6B7280]">Gerencie sua conta e preferências</p>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <Card>
                ...
                <CardContent>
                  {isEditingProfile ? (
                    <EditProfileForm user={user} onCancel={() => setIsEditingProfile(false)} onSuccess={handleProfileUpdate} />
                  ) : (
              ...
              <Card className="md:col-span-2">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle className="flex items-center gap-2">
                      ...
                      Gerenciar Membros
                      {!isUserAdmin && <Lock className="h-4 w-4 text-[#8B5CF6]" />}
                    </CardTitle>
                ...
                <CardContent className="space-y-4">
                  ...
                  {loadingMembers ? (
                    <>
                      <div className="h-20 bg-white border border-[#D8CCFF] rounded animate-pulse opacity-40" />
                      <div className="h-20 bg-white border border-[#D8CCFF] rounded animate-pulse opacity-40" />
                    </>
                  ) : (
                    <div className="members-management">
                      <MembersManagement members={companyMembers} currentUser={user} onMembersUpdate={handleMembersUpdate} />
                    </div>
                  )}
                </CardContent>
              </Card>
      ...
      {isAddingMember && <AddMemberForm isOpen={isAddingMember} onClose={() => setIsAddingMember(false)} onSuccess={handleMembersUpdate} currentUser={user} />}
    </div>
  )
}
